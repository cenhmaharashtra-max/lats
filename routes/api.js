...
router.post('/save', requireAuth, async (req,res) => {
  ...
  try {
    ...
    await ups('lats_projects',
      ['id','name','tafs_no','length','tpc','civil_cost','la_cost','total_land','avail_land','la_required','la_length','approval_date','assigned_users'],
      (d.projects||[]).map(p=>({
    ...
    // P3a
    await ups('lats_p3a',['id','project_id','village_id','ptype','notif_date','cala_post','cala_name','cala_mobile','is_addl'],
      (d.process3a||[]).map(x=>({id:x.id,project_id:x.projectId,village_id:x.villageId,ptype:x.type,
        ...
        cala_mobile:x.calaMobile||null,is_addl:x.isAdditional||false}))
    );
    ...
    // PJM
    await ups('lats_pjm',['id','project_id','village_id','jm_date','sheet_date'],
      (d.processJM||[]).map(x=>({id:x.id,project_id:x.projectId,village_id:x.villageId,jm_date:x.jmDate||null,sheet_date:x.sheetDate||null}))
    );
    ...
    // P3D
    await ups('lats_p3d',['id','project_id','village_id','notif_date'],
      (d.process3D||[]).map(x=>({id:x.id,project_id:x.projectId,village_id:x.villageId,notif_date:x.notifDate||null}))
    );
    ...
    // P3G
    await ups('lats_p3g',['id','project_id','village_id','notif_date'],
      (d.process3G||[]).map(x=>({id:x.id,project_id:x.projectId,village_id:x.villageId,notif_date:x.notifDate||null}))
    );
